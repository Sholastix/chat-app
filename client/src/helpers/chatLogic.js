...
// One of the users is current user when the other is collocutor.
// Function returns collocutor's name which will be the name of the current user's 1-on-1 chat.
...
// Function returns full info about collocutor in the current user's 1-on-1 chat.
...
export const isSameTime = (messages, message, index) => {
  try {
    const previousMessageTime = new Date(messages[index - 1]?.createdAt).toLocaleTimeString(navigator.language, {
      hour: '2-digit',
      minute: '2-digit',
    });

    const currentMessageTime = new Date(message.createdAt).toLocaleTimeString(navigator.language, {
      hour: '2-digit',
      minute: '2-digit',
    });
